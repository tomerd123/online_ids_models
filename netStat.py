...
class netStat:
    ...
    def __init__(self, HostLimit=255,HostSimplexLimit=1000):
        ...
        #HTs
        self.HT_BW_dir = af.incStatHT_2D(limit=100)
        ...
        self.BW = af.windowed_incStat(self.L_BW)

    def purgeOldRecords(self,curTime):
        cutoffWeight = 0.000001
        total_removed = 0
        total_removed = total_removed + self.HT_BW_dir.cleanOutOldRecords(cutoffWeight,curTime)
        total_removed = total_removed + self.HT_BW_dir.cleanOutOldRecords(cutoffWeight,curTime)
        total_removed = total_removed + self.HT_BW_dir.cleanOutOldRecords(cutoffWeight,curTime)
        total_removed = total_removed + self.HT_BW_dir.cleanOutOldRecords(cutoffWeight,curTime)
        total_removed = total_removed + self.HT_BW_dir.cleanOutOldRecords(cutoffWeight,curTime)
        total_removed = total_removed + self.HT_BW_dir.cleanOutOldRecords(cutoffWeight,curTime)
        print(str(total_removed) + " records removed from all hashtables.")
    ...
    def updateGetStats(self, IPtype, srcMAC,dstMAC, srcIP, srcProtocol, dstIP, dstProtocol, datagramSize, timestamp):
        # General Bandwidth
        BWstat = self.BW.updateAndGetStats(datagramSize,timestamp)

        # Directional Bandwidth
        direction = self.findDirection(IPtype,srcIP,dstIP,srcMAC,dstMAC)
        BWstat_dir = self.HT_BW_dir.updateGet_2D(direction[0],direction[1],datagramSize,timestamp,self.L_BW_dir)
    ...
    def getNetStatHeaders(self,isDirectional=False):
        BWstat_headers = ["BW_"+h for h in self.BW.getHeaders()]
        BWdirstat_headers = ["BW_dir_"+h for h in self.HT_BW_dir.getHeaders_2D(self.L_BW_dir)]
